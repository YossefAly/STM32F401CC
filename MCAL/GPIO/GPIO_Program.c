...
/******************************************************************************/
/* Func. Name : GPIO_voidUARTInit                                             */
/* I/p Arguments : Copy_Usartx Options: USART1, USART2, USART6                */
/* Return : Nothing                                                           */
/* Desc. : This Api allows the user to Set AF for USART Pins                  */
/******************************************************************************/

void GPIO_voidUARTInit(u8 Copy_Usartx){
	switch(Copy_Usartx){
		case 0:
			for (int i=24;i<27;i++){
				SET_BIT(GPIOB->AFRL,i);
			}
			CLR_BIT(GPIOB->AFRL,27);
			for(int i=28;i<31;i++){
				SET_BIT(GPIOB->AFRL,i);
			}
			CLR_BIT(GPIOB->AFRL,31);
			}
...
void GPIO_voidTIM1Init(void){
	SET_BIT(GPIOA->AFRH,0);
	for(int i=1;i<4;i++){
		CLR_BIT(GPIOA->AFRH,i);
	}
/*	SET_BIT(GPIOA->AFRH,4);
	for(int i=5;i<8;i++){
		CLR_BIT(GPIOA->AFRH,i);
	}*/
}
/******************************************************************************/
/* Func. Name : GPIO_voidAFInit                                               */
/* I/p Arguments : Copy_PortID,Copy_PinId,Copy_AFId                           */
/* Return : Nothing                                                           */
/* Desc. : This Api allows the user to Set AF for Any Pins                    */
/******************************************************************************/
void GPIO_voidAFInit(u8 Copy_PortId,u8 Copy_PinId,u8 Copy_AFId){
        if((Copy_PortId > IOC) || (Copy_PinId > PIN15) || (Copy_AFId > 15))
    {
        /*Do Nothing*/
    }
    else{
    switch (Copy_PortId)
    {
        case IOA:
            if(Copy_PinId<8){
                GPIOA->AFRL&=~(0x0F<<Copy_PinId*4);
                GPIOA->AFRL|=(Copy_AFId<<Copy_PinId*4);
            }    
            else{
                GPIOA->AFRH&=~(0x0F<<Copy_PinId*4);
                GPIOA->AFRH|=(Copy_AFId<<Copy_PinId*4);
            }
            break;
        case IOB:
                        if(Copy_PinId<8){
                GPIOB->AFRL&=~(0x0F<<Copy_PinId*4);
                GPIOB->AFRL|=(Copy_AFId<<Copy_PinId*4);
            }    
            else{
                GPIOB->AFRH&=~(0x0F<<Copy_PinId*4);
                GPIOB->AFRH|=(Copy_AFId<<Copy_PinId*4);
            }
            break;
        case IOC:
                            if(Copy_PinId<8){
                GPIOC->AFRL&=~(0x0F<<Copy_PinId*4);
                GPIOC->AFRL|=(Copy_AFId<<Copy_PinId*4);
            }    
            else{
                GPIOC->AFRH&=~(0x0F<<Copy_PinId*4);
                GPIOC->AFRH|=(Copy_AFId<<Copy_PinId*4);
            }
            break;    
        default:
            break;
